...
export default function InstallPrompt() {
  ...
  return (
    <div
      className="fixed top-0 left-0 right-0 z-banner bg-primary-600 text-white shadow-lg"
      style={{ paddingTop: 'env(safe-area-inset-top)' }}
      role="alert"
    >
      <div className="px-3 sm:px-4 py-2.5 flex items-center gap-2 text-sm">
        <Icon icon={Smartphone} size={20} className="shrink-0" />
        <div className="flex-1 min-w-0">
          <div className="font-medium">添加到主屏幕</div>
          <div className="text-xs text-primary-100">
            点击底部分享按钮 <Icon icon={Share} size={12} className="inline-block align-[-2px]" /> →
            "添加到主屏幕",像 App 一样使用
          </div>
        </div>
        <button
